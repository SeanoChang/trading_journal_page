    ...
    const load = async () => {
      ...
      try {
        const [gRes, oRes] = await Promise.all([
          ...
          fetch(
            `/api/ohlc?symbol=${encodeURIComponent(symbol)}&interval=1d`,
  ...
  return (
    ...
    >
      <motion.div variants={itemVariants} className="md:col-span-1">
        ...
        <GreedGauge
          value={greed?.value ?? null}
          label={greed?.classification ?? "—"}
          loading={loading}
        />
      </motion.div>
          ...
          >
            {Array.from(
              new Set([
                "BTCUSDT",
                "ETHUSDT",
                ...favorites.map((f) => `${f.toUpperCase()}USDT`),
              ]),
            ).map((s) => (
...
function GreedGauge({
  ...
  const color =
    v < 30 ? "text-rose-500" : v > 70 ? "text-emerald-500" : "text-amber-500";
  return (
      ...
      {!loading && (
        <div>
          <div className="h-2 w-full rounded bg-slate-200/60 dark:bg-slate-800/60 overflow-hidden">
            <div
              className="h-full bg-gradient-to-r from-rose-500 via-amber-500 to-emerald-500 transition-all duration-700 ease-out"
              style={{ width: `${pct}%` }}
            />
          </div>
...
function MiniSpark({ data }: { data: number[] }) {
  const w = 160,
    ...
    pad = 4;
  if (!data?.length)
    return <div className="text-xs text-default-500">No data</div>;
