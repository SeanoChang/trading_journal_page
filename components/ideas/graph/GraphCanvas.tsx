...
export const GraphCanvas = forwardRef<HTMLCanvasElement, GraphCanvasProps>(
  (
    ...
    useEffect(() => {
      ...
      const render = () => {
        ...
        if (isolatedNodeRef.current) {
          ...
          filteredNodes = filteredNodes.filter(
            (node) => node.id === isolatedId || connectedNodes.has(node.id),
          );
        }

        const hoveredConnections = hoveredNodeRef.current
          ? getConnectedNodes(hoveredNodeRef.current.id)
          : new Set<string>();
        ...
        const filteredLinks = links.filter((link) => {
          ...
          switch (linkMode) {
            case "temporal":
              return link.kind === "temporal";
            case "conceptual":
              return link.kind === "topic" || link.kind === "tag";
            case "hierarchy":
              return link.kind === "hierarchy";
            case "ticker":
              return link.kind === "ticker";
            case "all":
              return true;
            default:
              return true;
        ...
        filteredLinks.forEach((link) => {
          ...
          const targetId =
            typeof link.target === "string" ? link.target : link.target.id;
          const isHighlighted =
            hoveredConnections.has(sourceId) ||
            hoveredConnections.has(targetId);
          ...
          if (link.kind === "hierarchy") {
            ...
            context.lineTo(
              arrowX - (arrowSize * dx) / length + (arrowSize * dy) / length,
        ...
        sortedNodes.forEach((node) => {
          ...
          const isConnected =
            hoveredNodeRef.current && hoveredConnections.has(node.id);
          ...
          context.strokeStyle = isHighlighted
            ...
            : isDragged
              ...
              : isHovered || isConnected
                ? "#f59e0b"
                : node.type === "tag"
                  ? getNodeColor(node, isTimelineMode)
                  : "#1e293b";
          context.lineWidth =
            isHighlighted || isHovered || isDragged
              ? 3
              : isConnected
                ? 2
                : node.type === "tag"
      ...
      canvasElement.addEventListener("wheel", preventScroll, {
        passive: false,
      });
