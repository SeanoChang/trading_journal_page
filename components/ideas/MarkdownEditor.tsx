...
function mdToHtml(md: string) {
  ...
  html = html.replace(
    /```([\s\S]*?)```/g,
    (_, code) =>
      `<pre class="rounded bg-slate-950/90 text-slate-100 p-3 overflow-auto"><code>${code}</code></pre>`,
  );
  ...
  html = html.replace(
    /`([^`]+)`/g,
    (_, code) =>
      `<code class="bg-slate-200 dark:bg-slate-800 px-1 rounded">${code}</code>`,
  );
  ...
  html = html.replace(
    /^#####\s?(.*)$/gm,
    '<h5 class="text-base font-semibold mt-4 mb-1">$1<\/h5>',
  );
  html = html.replace(
    /^####\s?(.*)$/gm,
    '<h4 class="text-lg font-semibold mt-4 mb-1">$1<\/h4>',
  );
  html = html.replace(
    /^###\s?(.*)$/gm,
  ...
  html = html.replace(
    /^#\s?(.*)$/gm,
    '<h1 class="text-3xl font-bold mt-6 mb-3">$1<\/h1>',
  );

  // 4) links [text](url)
  html = html.replace(
    /\[([^\]]+)\]\(([^)\s]+)\)/g,
    '<a class="text-emerald-600 hover:underline" target="_blank" rel="noopener noreferrer" href="$2">$1<\/a>',
  );
  ...
  for (const line of lines) {
    ...
    if (ul) {
      if (!inUl) {
        closeLists();
        out.push('<ul class="list-disc ml-5 my-2 space-y-1">');
        inUl = true;
      }
      out.push(`<li>${ul[1]}</li>`);
      continue;
    ...
    if (ol) {
      if (!inOl) {
        closeLists();
        out.push('<ol class="list-decimal ml-5 my-2 space-y-1">');
        inOl = true;
      }
      out.push(`<li>${ol[1]}</li>`);
      continue;
    ...
    <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white/90 dark:bg-slate-900/80 p-4">
      <div className="flex flex-wrap items-center gap-2 mb-3">
        ...
        <div className="ml-auto flex items-center gap-2">
          <Button size="sm" variant="flat" onPress={() => setImmersive(true)}>
            Immersive
          </Button>
          <Button
            size="sm"
            variant="flat"
            onPress={() => setSplit((s) => (s === "split" ? "stack" : "split"))}
          >
            {split === "split" ? "Stack" : "Split"}
          </Button>
