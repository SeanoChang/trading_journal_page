...
function htmlToMarkdown(html: string) {
  ...
  out = out.replace(/<p>([\s\S]*?)<\/p>/g, (_: string, c: string) => c + "\n\n");
  out = out.replace(/<h1[^>]*>([\s\S]*?)<\/h1>/g, (_: string, c: string) => `# ${c}\n\n`);
  out = out.replace(/<h2[^>]*>([\s\S]*?)<\/h2>/g, (_: string, c: string) => `## ${c}\n\n`);
  out = out.replace(/<h3[^>]*>([\s\S]*?)<\/h3>/g, (_: string, c: string) => `### ${c}\n\n`);
  out = out.replace(/<blockquote[^>]*>([\s\S]*?)<\/blockquote>/g, (_: string, c: string) => c.split(/<br\s*\/?/).map((l: string) => `> ${l}`).join("\n") + "\n\n");
  // lists
  out = out.replace(/<ul[^>]*>([\s\S]*?)<\/ul>/g, (_: string, li: string) => li.replace(/<li[^>]*>([\s\S]*?)<\/li>/g, (_: string, t: string) => `- ${t}\n`) + "\n");
  out = out.replace(/<ol[^>]*>([\s\S]*?)<\/ol>/g, (_: string, li: string) => {
    let i = 1;
    return li.replace(/<li[^>]*>([\s\S]*?)<\/li>/g, (_: string, t: string) => `${i++}. ${t}\n`) + "\n";
  });
  // inline
...
export default function InlineMarkdownEditor({ initial, onSave, onExit, showExit = true }: { initial?: { title?: string; content?: string; date?: string }; onSave?: (d: { title: string; content: string; date: string }) => void; onExit?: () => void; showExit?: boolean }) {
  ...
  useEffect(() => {
    ...
    if (initial?.content) {
      ...
      el.innerHTML = lines.map((l) => (l.trim() === "" ? "<div><br></div>" : `<div>${l}</div>`)).join("");
    }
  ...
  const applyBlockShortcuts = () => {
    ...
    while (container && (container as HTMLElement).nodeType === 3 ? (container.parentElement && container.parentElement.tagName !== 'DIV') : (container as HTMLElement).tagName !== 'DIV') {
  ...
  const applyInlineShortcuts = () => {
    ...
    while ((node = walker.nextNode())) {
      ...
      if (/\*\*[^*]+\*\*/.test(t.nodeValue) || /\*[^*]+\*/.test(t.nodeValue) || /`[^`]+`/.test(t.nodeValue)) {
        candidates.push(t);
      }
    }
  ...
  const transformBlock = (kind: 'text' | 'h1' | 'h2' | 'h3' | 'ul' | 'ol' | 'quote' | 'divider') => {
    const el = editorRef.current;
    if (!el) return;
    const sel = window.getSelection();
    if (!sel || sel.rangeCount === 0) return;
    const range = sel.getRangeAt(0);
    let container = range.startContainer as HTMLElement | Text;
    while (container && !(container instanceof HTMLElement && container.tagName === 'DIV')) {
      const parent = (container as any).parentElement as HTMLElement | null;
      if (!parent) break;
      container = parent;
    }
    ...
    <div className="h-full flex flex-col">
      <div className="flex items-center gap-2 p-3 border-b border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-slate-900/80 backdrop-blur">
        ...
        <div className="ml-auto flex items-center gap-2">
          {showExit && <Button size="sm" variant="flat" onPress={onExit}>Exit</Button>}
      ...
      <div className="flex-1 overflow-auto">
        ...
        {slashOpen && (
          ...
          >
            ...
            <MenuItem label="Bulleted list" onClick={() => transformBlock('ul')} />
            <MenuItem label="Numbered list" onClick={() => transformBlock('ol')} />
            <MenuItem label="Quote" onClick={() => transformBlock('quote')} />
            <MenuItem label="Divider" onClick={() => transformBlock('divider')} />
          </div>
        )}
      </div>
    </div>
