...
export const analyzeSentiment = (text: string): 'positive' | 'negative' | 'neutral' => {
  const positiveWords = ['gain', 'profit', 'bull', 'rise', 'up', 'surge', 'rally', 'growth'];
  const negativeWords = ['loss', 'drop', 'bear', 'fall', 'down', 'crash', 'decline', 'dump'];
  
  const lowercaseText = text.toLowerCase();
  const positiveCount = positiveWords.reduce((count, word) => 
    count + (lowercaseText.split(word).length - 1), 0);
