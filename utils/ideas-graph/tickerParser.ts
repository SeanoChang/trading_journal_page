...
export const cryptoRegex = /\b(BTC|ETH|ADA|SOL|DOT|LINK|MATIC|AVAX|ATOM|UNI|AAVE|MKR|COMP|YFI|SNX|1INCH|CRV|BAL|SUSHI|ZRX|LRC|GRT|BAND|REN|KNC)\b/gi;
...
export function analyzeTickerSentiment(text: string, ticker: string): 'bullish' | 'bearish' | 'neutral' {
  const lowerText = text.toLowerCase();
  const tickerMentions = text.match(new RegExp(`\\$${ticker}|\\b${ticker}\\b`, 'gi')) || [];
  
  if (tickerMentions.length === 0) return 'neutral';
  
  const bullishWords = [
    'buy', 'long', 'bull', 'bullish', 'up', 'rise', 'moon', 'pump',
    'strong', 'support', 'breakout', 'rally', 'surge', 'gain',
    'positive', 'optimistic', 'confidence', 'uptrend', 'higher'
  ];
  
  const bearishWords = [
    'sell', 'short', 'bear', 'bearish', 'down', 'fall', 'dump',
    'weak', 'resistance', 'breakdown', 'crash', 'drop', 'loss',
    'negative', 'pessimistic', 'doubt', 'downtrend', 'lower'
  ];
...
export function getAssetClassColor(ticker: string): string {
  // Tech stocks
  if (['AAPL', 'GOOGL', 'MSFT', 'META', 'AMZN', 'TSLA', 'NVDA', 'AMD'].includes(ticker)) {
    return '#3b82f6'; // Blue
  }
