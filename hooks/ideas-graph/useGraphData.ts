...
import type { Idea, GraphData, IdeaNode, TagNode, Link } from '../../types/ideas-graph';
import { parseNestedTags, calculateAge, processJournalEntry } from '../../utils/ideas-graph';
  ...
  return useMemo(() => {
    ...
    ideas.forEach((idea) => {
      ...
      const ideaNode: IdeaNode = {
        ...
        type: 'idea',
        degree: 0,
        radius: Math.max(6, Math.min(12, 6 + (idea.tags?.length || 0) * 0.5 + (processedIdea.tickers?.length || 0) * 0.8)),
        age
      };
    ...
    const sortedIdeas = [...ideaNodes].sort((a, b) => 
